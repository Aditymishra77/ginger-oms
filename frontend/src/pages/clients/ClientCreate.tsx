...
export function ClientCreate() {
  const { register, control, handleSubmit, formState: { errors, isSubmitting }, setError, clearErrors } = useForm<FormValues>({
    defaultValues: {
      name: '',
      taxId: '',
      salesRepId: '',
      contacts: [{ firstName: '', lastName: '', email: '', phone: '', role: 'PRIMARY', isPrimary: true }],
      addresses: [{ type: 'BOTH', addressLine1: '', addressLine2: '', city: '', state: '', postalCode: '', country: 'India', isDefault: true }]
    }
  });
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      setError('root', { type: 'server', message: err.response?.data?.error || 'Failed to create client' });
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {errors.root?.message && (
        <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-lg text-sm">{errors.root.message}</div>
      )}
                ...
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="flex items-center space-x-3">
                    <label className="flex items-center text-sm text-gray-600 space-x-1">
                      <input type="checkbox" {...register(`contacts.${idx}.isPrimary`)} className="rounded" />
                      <span>Primary</span>
                    </label>
